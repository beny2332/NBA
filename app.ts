document.addEventListener("DOMContentLoaded", () => {
  ...
  const selectPositionElm: HTMLSelectElement =
    document.querySelector("select")!;
  const pointsInputElm: HTMLInputElement =
    document.querySelector("#pointsInput")!;
  const twoPercentInputElm: HTMLInputElement =
    document.querySelector("#twoPercentInput")!;
  const threePercentInputElm: HTMLInputElement =
    document.querySelector("#threePercentInput")!;
  const searchButton: HTMLButtonElement =
    document.querySelector("#search-btn")!;
  const tableBody: HTMLTableSectionElement = document.querySelector("tbody")!;

  const playerDivs: { [key: string]: HTMLElement } = {
  ...
  const searchPlayers = async (): Promise<void> => {
    ...
    try {
      ...
      const resArray: resBody[] = await res.json();
  ...
  const displayResults = (players: resBody[]): void => {
    tableBody.innerHTML = "";
    players.forEach((player) => {
      ...
      addPlayerBtn.textContent = `add ${
        player.playerName.trim().split(" ")[0]
      } to current team`;
  ...
  const addPlayerToTeam = (player: resBody): void => {
    ...
    if (playerDiv) {
      ...
      if (playerInfoDiv) {
        ...
        console.log(
          `Added ${player.playerName} as the team's ${player.position}`
        );
      } else {
